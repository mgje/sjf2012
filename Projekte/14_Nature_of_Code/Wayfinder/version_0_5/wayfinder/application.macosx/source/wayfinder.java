...
public void render_environment(){
  ...
  switch(mode){
    ...
    text("distance: "+ways[todraw].way_distance*(386.0f/920.0f) + " km", 10, 100);
    break;
...
public float distance(int x_pos1, int y_pos1,int x_pos2, int y_pos2){
  float result;
  result = sqrt(pow((float)x_pos1-(float)x_pos2,2.0f) + pow((float)y_pos1-(float)y_pos2,2.0f));
  return result;
}
...
public float calc_way(int[] reihenfolge)
{
 ...
 //**** Funktion int fitness() ***********************************************
  ...
  //
  //
  public int[] random_order(int nr_of_objects){
 ...
 // der nachfolgende Algorithmus versucht \u00dcberkreuzungen von Wegen zu reduzieren
  ...
  //  
  public way reduce_crossover(way temp_way){
    ...
    steigung_first = ((((float)kunden[order[0]].y_pos)-255.0f))/(((float)kunden[order[0]].x_pos)-545.0f);
    offset_first = (float)kunden[order[0]].y_pos - steigung_first*(float)kunden[order[0]].x_pos;
    
    steigung_last = ((((float)kunden[order[array_length]].y_pos)-255.0f))/(((float)kunden[order[array_length]].x_pos)-545.0f);
    ...
    for(int i = 0; i<array_length; i++){
        ...
        if(kunden[order[i]].x_pos > kunden[order[i+1]].x_pos){
          if(545 > kunden[order[0]].x_pos){
            if((float)kunden[order[i]].x_pos-cross_tol >= cross_x && (float)kunden[order[i+1]].x_pos+cross_tol <= cross_x){
              if(545.0f-cross_tol >= cross_x && (float)kunden[order[0]].x_pos+cross_tol <= cross_x){
              found = 1;
              }
            }
          }
        ...
        if(kunden[order[i]].x_pos > kunden[order[i+1]].x_pos){
          if(545 > kunden[order[array_length]].x_pos){
            if((float)kunden[order[i]].x_pos-cross_tol >= cross_x && (float)kunden[order[i+1]].x_pos+cross_tol <= cross_x){
              if(545.0f-cross_tol >= cross_x && (float)kunden[order[array_length]].x_pos+cross_tol <= cross_x){
              found = 1;
              }
            }
          }
  ...
  public int check_cross(int[] order, int set_i, int array_length){
  float cross_x; 
  int i = set_i;
  int found = 0;
    for(int j = 0; j<array_length; j++){
  ...
  //  
  public void calc_steigung(int arr_length, int[] order){
...
//**************evolution end*******************************************************************************
 ...
 //************** Class kunde  - End*****************************************************************************
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "wayfinder" };
    if (passedArgs != null) {
