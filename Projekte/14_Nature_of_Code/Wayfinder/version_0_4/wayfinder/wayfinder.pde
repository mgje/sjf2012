    ...
    int badest=11;
     ...
     for(int i = 0; i<numb_ways; i++){
       /*for(int k = 0; k<(int)random(0,3); k++){
         ways2[i] = reduce_crossover(ways2[i]);
       }*/
              ...
              /*case 0:
                  ...
                  for(int h=0; h<3; h++){
                    switch(h){
                      ...
                      temp_pos = order[rand];
                      order[rand] = order[rand2];
                      order[rand2] = temp_pos;
                      calc_steigung(array_length, order);
                      if(check_cross(order,i, array_length) == 0){
                        success = 1;
                        break;
                      }
                      if(success == 1){
                  ...
                  if(success == 0){
                    ...
                    calc_steigung(array_length, order);
                    rand = (int)random(0,1);
                    rand2 = (int)random(0,1);
                    /*temp_pos = order[i+rand];
                    order[i+rand] = order[j+rand2];
                    order[j+rand2] = temp_pos;*/
                  }
              
                break;*/
              /*case 1:
                if(kunden[order[i]].x_pos > kunden[order[i+1]].x_pos && kunden[order[j]].x_pos > kunden[order[j+1]].x_pos){
                  temp_pos = order[i];
                  order[i] = order[j];
                  order[j] = temp_pos;
                }
                else if(kunden[order[i+1]].x_pos > kunden[order[i]].x_pos && kunden[order[j]].x_pos > kunden[order[j+1]].x_pos){
                  temp_pos = order[i+1];
                  order[i+1] = order[j];
                  order[j] = temp_pos;
                }
                else if(kunden[order[i]].x_pos > kunden[order[i+1]].x_pos && kunden[order[j+1]].x_pos > kunden[order[j]].x_pos){
                  temp_pos = order[i];
                  order[i] = order[j+1];
                  order[j+1] = temp_pos;
                }
                else if(kunden[order[i+1]].x_pos > kunden[order[i]].x_pos && kunden[order[j+1]].x_pos > kunden[order[j]].x_pos){
                  temp_pos = order[i+1];
                  order[i+1] = order[j+1];
                  order[j+1] = temp_pos;
                }
              ...
              /*case 0:
                for(int k = 0; k<4; k++){
                  ...
                  for(int h = k + 1; h< 4; h++){
                    ...
                    temp_distance = distance(kunden[eins].x_pos, kunden[eins].y_pos, kunden[zwei].x_pos, kunden[zwei].y_pos);
